...
- `section` 必須是 `SECTIONS` 內的值，**但留空是允許的**。打錯字（少一個「的」）
  不擋的話會被 digest 歸進「其他」節而從該去的位置靜默消失；留空則必須放行，
  因為 2026-07 有 30 則歷史資料沒填，擋下來會讓 `import-json --replace`
  還原不了它們，而 `data/news.json` 必須是 db 的完整鏡像。
...
python3 news.py serve [--port 8765] [--host tailscale]  # 網頁介面（--host 見下，預設只有本機可連）
...
**實測結論**（完整數據與三輪實驗見 `docs/second-opinion-findings.md`，
改 criteria 或擴充 Jev 用途前先讀）：

- 真實排序能力在同等級內約 0.43-0.66，**報告時不要引用 0.893**（那是跨等級樣本撐高的）。
- `decision` 排序最可靠，但絕對水準系統性低約 1.8 分——要讓 Jev 接手評分，
  該修的是 `decision` 的 criteria，不是餵它更多內容（餵內文反而略差）。
- `confidence` 不能當單則誤差的預測值，只在面向層級有意義。
- 套外樣本證實它測得到漂移：7 月漂移期 decision 差值是其他兩組的 1.8-2.5 倍。
- **擴充 Jev 用途前先問：任務需要的資訊在它的輸入裡嗎？** 分 `section`、
  預篩 pending 都因此否決（它讀程度、不讀門檻）。
...
- **依等級的命中率不能直接當成評分鬆緊的證據**——這條在 2026-08-02 被推翻，
  原本寫「高分級若低於低分級就是評分過鬆的直接證據」，但那個推論的前提
  （各等級的指標難度相同）不成立。詳見下方「等級命中率為何不可直接比較」。
...
- **寫指標前先確認它問的事情是否已經發生**。2026-08-29 寫下「歐盟或其他
  司法管轄區是否要求強制標示 AI 生成內容」，而歐盟的規定 **08-02 就已生效**
  ——寫的當下它已經是事實，只是我不知道。這種條目判 hit 會讓命中率含進
  「查核不完整」而非「預測準確」，但它既不是 miss（那件事確實發生了）
  也不是 moot（前提沒有消失），三個值都不對。**成本最低的處置是不要寫出來**：
  問「這個規定／協議／數字是不是已經公布了」比事後分類便宜。
  （投資線的 `void` 正是這一類的歸因，但刻意不給新聞線用——見「投資觀察」。
  若哪天累積到三五條同類再談，為單一案例改 schema 不成比例。）
...
**等級命中率為何不可直接比較（2026-08-02 修正）**

2026-08-02 的統計出現 **B 52%（35/67）> A 42%（23/55）**，依原本的規則
應判定為「評分過鬆」。實際查看指標內容後發現不是——**等級與指標型態高度相關**，
命中率差異反映的是指標難度而非判讀能力。
...
| | A 級的 miss | B 級的 hit |
|---|---|---|
| 型態 | 制度性事件（一次性正式動作） | 持續性指標（既有數字的變動） |
| 例子 | 「政府是否尋求**國會立法**重建關稅權」<br>「60 天免費期屆滿後伊朗**正式公布**的費率」<br>「哪些國家被**列入**友好國」<br>「亞利桑那州各期廠的**實際量產時程**」 | 「台股融資餘額與斷頭壓力」<br>「記憶體報價與 SK 海力士基本面」<br>「俄國煉油產能受損程度」<br>「布蘭特是否站上 80/90 美元」 |
| 為何如此 | A 級的定義是影響範圍廣、結構性意義大，寫指標時自然去問「這個結構會不會真的改變」（立法、公告、簽約）——而制度性改變本來就慢，30 天窗口太短 | B 級多是產業或市場新聞，指標落在「數字會不會繼續動」，只要主題還在報導就會命中 |

這其實是上面那條「指標寫法的影響比等級更大」的必然結果：
**A 級的 miss 就是特定事件型（29%），B 級的 hit 就是機制延續型（48%）**，
先前只是沒把這兩件事連起來。

- **不要因為 B > A 就調整評分標準**。要比較等級的判讀能力，得先控制指標型態
  （只比同型態的命中率），或延長 A 級指標的觀察窗口。
- **可行的改善是在寫 A 級指標時刻意混入機制延續型**：現在 A 級幾乎全是
  制度性事件，導致命中率結構性偏低。同一則裡放一條「這個主題的既有指標
  會怎麼動」，兩種型態都有才測得出東西。
- 首輪（2026-07-28）的 A 42% > B 33% > C 14% 之所以梯度正確，可能只是當時
  A 級的樣本還沒累積到足夠多的制度性指標，**不要把那次的數字當成基準**。
...
- **`serve` 預設綁 `127.0.0.1`，改動預設值要是刻意的**
  （由 `TestServeBindsLocalByDefault` 守著）。**風險由使用者決定何時承擔，
  不由預設值替他決定**，所以不要為了方便把預設值改掉。
  要用手機看有兩種：
  - **`--host tailscale`（建議）**：自動查出 Tailscale 位址（`tailscale ip -4`），
    只有自己 tailnet 內的裝置連得到，即使在公共 Wi-Fi 同網段的人也看不到。
  - `--host 0.0.0.0`：同網段所有裝置都看得到 `/positions` 且沒有密碼。
    兩者的警告訊息刻意不同——印同一段會讓人以為一樣危險，反而降低可信度。
- **`server_bind` 必須跳過反向 DNS 查詢**（自訂的 `Server` 類別，由
  `TestServeBindDoesNotHangOnDNS` 守著）。2026-07-31 實測：綁 Tailscale 位址時
  `HTTPServer.server_bind()` 內的 `socket.getfqdn()` **不回應也不超時**，
  程序活著但 port 從未開始監聽——症狀是「啟動成功但連不到」，極難診斷
  （最後靠 `faulthandler.dump_traceback_later` 設逾時才抓到堆疊）。
...
**要跑完整每日流程（fetch → 評分 → watch → position → commit+push）時用
`/update-news`**，不要逐步手打。使用者說「批次更新新聞」「批次更新新聞和投資」
「批次更新新聞和財務」等變體，指的都是這一套。本節只涵蓋其中的評分那一步。
...
- `docs/second-opinion-findings.md` — 第二意見（Jev）的三輪實測數據，
  CLAUDE.md 只留結論
