    ...
    if acc["by_grade"]:
        ...
        for g in GRADES:
            ...
            print(f"  {g} 級  命中率 {gc['rate']:.0%}"
                  f"（{gc['counts']['hit']}/{gj}）")
        # 2026-08-02 修正：原本這裡印「高分級低於低分級就是評分過鬆」，
        # 但實測 B 反超 A 後查看指標內容，發現是等級與指標型態相關所致——
        # A 級傾向寫制度性事件（立法、公告，30 天窗口太短），B 級傾向寫
        # 持續性指標（價格、產量，只要主題還在報導就會命中）。
        # 印一句會被誤用的結論比不印更糟，改成提醒前提。
        print("\n  ⚠️  等級間的命中率不可直接比較：A 級傾向寫制度性事件"
              "（立法、公告），")
        print("     B 級傾向寫持續性指標（價格、產量），後者本來就容易命中。")
        print("     差異多半反映指標型態而非判讀能力，別據此調整評分標準。")
