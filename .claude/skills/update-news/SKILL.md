...
### 步驟 5：投資觀察（sub-agent 平行處理）

**這一步派一個 sub-agent 做，與步驟 3、4 平行。** 理由是它需要讀完整批
已評分新聞 + 全部既有觀點 + 到期預測清單，那是大量只用一次的中間資料——
留在主線程會把後面的收尾擠掉，而它的產出只有「寫了什麼」這幾行有用。

用 Agent tool（`subagent_type: "general-purpose"`）派下去，一次交代兩件事：

**5a. 新增觀點**（`add-position`，`author` 必須填 `"agent"`）

門檻是**出現能改變既有判斷的新資訊**，不是每天都要寫。實測 2026-08-03
第二批寫 0 則是對的——那天唯一評到的是撤回性質的新聞，不構成新判斷依據。
**沒有就寫 0 則，不要為了有產出而硬湊。**

最有價值的是**修正既有觀點**：#10 修正 #8 修正 #3、#13 承認 #1 的門檻設太寬鬆。
所以 agent 必須先 `python3 news.py positions -v --limit 999` 讀完**全部**既有觀點，
才知道今天的新聞跟哪一則衝突。`--limit` 預設只給 20 筆且截斷提示混在長篇輸出裡，
漏讀不會報錯——要讀全部就得明寫。

**5b. 到期預測判定**（`position-due` → `position-verify`）

判定前先問：**這條證據能不能直接回答預測問的問題？** 不能就別判。
到期清單多數在等財報公布，**回報「資料未出生」是正確答案**，不是失職。

`position-due` 不截斷，且**只列真正該判的**：過了 `due_date` 的，加上沒填
`due_date` 而放滿 14 天的。填了 `due_date` 但期限未到的預設不列（`--all` 可看），
只印一行條數——2026-09-20 實測那 47 條「到期」裡真正過期的是 0 條，
每天列出同一批註定「資料未出生」的條目，兩週後這張清單就會被跳過。
所以**清單空的是正常結果**，不要因為看到 0 條就去翻 `--all` 硬判。
`positions` 仍是預設 20，讀既有觀點要自己加 `--limit 999`。

⚠️ **`--force` 不准用**。改判定會讓命中率失去意義，agent 不得自行改寫
既有判定；發現判錯了就回報，由人決定。

#### author 標記為什麼是強制的

`add-position` 的 `author` 欄位（2026-09-08 加入）決定命中率算進誰頭上，
`position-stats` 依它分開列出「自己」與「Agent」兩組數字。

**這條線的設計目的是校準「我的判斷準不準」**（見 `CLAUDE.md` 的「投資觀察」），
而 agent 自動寫入的觀點測的是模型的判斷。兩者混算，那個數字就同時含人與模型：
**好看不知道是誰準，難看也不知道該檢討誰**——整條校準線就失效了。

漏填 `author` 的話 CLI 會靜默當成 `me`（那是舊資料的正確預設值），
不會報錯，所以**這件事只能靠 prompt 交代清楚**。

#### 這個安排的已知代價

使用者 2026-09-08 明確選擇了全自動寫入，知道並接受下面這點：

> `CLAUDE.md` 原本的設計是「公開投資判斷會改變書寫方式——會不自覺寫得保守、
> 寫得容易命中，而這條線的全部價值就在於記錄真實的判斷」。全自動寫入後，
> **人的判斷軌跡不會再自動累積**（08-06 之後就已經停了）。

所以 agent 的產出**不能取代**你自己寫觀點。想恢復人的那條線，就在看完
agent 回報後自己補寫（不填 `author` 即可）。
...
- **投資觀察資料不得進版控、不得上靜態站**（`CLAUDE.md` 的「投資觀察」，`TestPositionsStayLocal`
  守著）。步驟 5 的產出（含 sub-agent 寫入的）只留在本機 `news.db`，
  `export-json` 只匯出 news 表。
...
- **agent 寫觀點必須填 `author: "agent"`**。漏填會被靜默當成人寫的
  （那是舊資料的正確預設），CLI 不報錯，於是模型的判斷就混進了你的命中率。
  這是這條線的前提問題，見步驟 5。
